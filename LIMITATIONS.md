...
Checkpoint seven is a local read-only research API, not a public production service. It has no authentication, rate limiting, TLS termination, deployment hardening, or availability guarantee. Search is PostgreSQL `ILIKE`; pagination remains offset-based but uses deterministic total ordering. Network endpoints expose contemporaneous staff edges and their evidence status without building or interpreting a graph. An edge can be filtered only when both assignments share the requested verification status; mixed-status edges remain visible without that filter. PostgreSQL timestamps can differ between clean loads; analytical view content and version identities cannot. A manual input edited during a load causes that candidate transaction to fail and must be rerun from a fresh byte snapshot. The API serves the same exploratory/suppressed coach outputs from checkpoint six and does not improve their identification.

## Frontend

Checkpoint eight is a local analytical interface, not a deployed product. It inherits all checkpoint-seven completeness and identification limits and does not add authentication, caching infrastructure, monitoring, or a public availability guarantee. Coach filtering on the statistics route requires a client-side intersection of complete paginated assignment and QB responses because the API has no joined QB/coach search endpoint; this is acceptable for the current publication but would need a server-side query for materially larger datasets. Network team hubs are presentation context derived from assignment edges, not evidence of influence, mentorship, or causation. Connected quarterbacks on coach pages indicate shared team-season context only, not direct coaching exposure.
