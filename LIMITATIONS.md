...
The production API is a public, read-only research service with provider TLS and exact-origin CORS, but it is intentionally unauthenticated and has no application rate limiter or availability SLA. Render and Neon free tiers can cold-start, throttle, or become unavailable. Search is PostgreSQL `ILIKE`; pagination remains offset-based but uses deterministic total ordering. The Relationship Explorer is bounded to 1,000 nodes and 2,000 relationships, and Full Network requests are limited to an anchored five-season window; it is not a bulk-history export. Its coach-to-team-season and QB-to-team-season path establishes season context only, not exact weekly overlap or causation. Deterministic screen positions and highlighted paths are navigation aids, not modeled relationships. Dense valid scopes can return 413 and must be narrowed; no partial graph is shown. On small screens the graph is visually constrained, so the metadata-complete relationship list is the primary equivalent. PostgreSQL timestamps can differ between clean loads; analytical view content and version identities cannot. A manual input edited during a load causes that candidate transaction to fail and must be rerun from a fresh byte snapshot. The API serves the same exploratory/suppressed coach outputs from checkpoint six and does not improve their identification.
