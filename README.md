...
Checkpoint 19 adds an isolated, **not deployed** Ask Anything interface to approved
history, C14 state/style, C13 scheme, C16 team-independent research projections and
...
The serving publication implements schema `checkpoint-7.5`, loader `serving-loader-v7`, and API contract `api-v1.5`, returning deterministic `Coach -> Team-Season <- QB` facts. Coach-assignment relationships remain one `assignment_key`; QB relationships remain one `(player_id, team_id, season)` and receive PAE only through that complete key. Team-history and team-anchored Full Network requests seed canonical-position QB facts independently from `api_qb_statistics`, so role, verification, and provisional filters affect coach edges without deleting valid QB-team-season facts. Coach/QB/team-history modes require the corresponding anchor. Full Network can request the complete 2010–2025 scope without an anchor; ordinary modes retain 1,000-node/2,000-relationship caps and Full Network uses measured 2,000-node/4,000-relationship caps. No response is silently truncated. The response labels coach-QB context as same-team-season context, not exact weekly exposure or causation. Coaching completeness is rebuilt from the finalized Eleven-B evidence: a `verified_no_designated_role` cell means the sourced evidence verifies that no separate person held the role, so the API exposes provenance without inventing a coach or assignment.
...
## Build checkpoint thirteen

With the ignored Checkpoint 3 and frozen Checkpoint 12 artifacts available locally:

```bash
make PYTHON=.venv/bin/python checkpoint-thirteen
```

The command writes content-addressed files under
...
- [Checkpoint 13 predictive data foundation](docs/CHECKPOINT_13_PREDICTIVE_DATA_FOUNDATION.md)
- [Predictive feature dictionary](docs/PREDICTIVE_FEATURE_DICTIONARY.md)
