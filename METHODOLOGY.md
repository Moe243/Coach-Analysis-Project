...
Checkpoint seven does not recompute metrics or models. It verifies every upstream checksum, required column, business key, lineage, interval, citation, fractional exposure, and model/data version before insertion. All tables and the publication-pointer swap share one PostgreSQL transaction. The load identity hashes schema/loader/API versions and all upstream analytical identities. Independent empty databases must produce the same load ID and identical ordered JSON checksums for every serving view; database timestamps are execution metadata and may differ.
