...
Checkpoint four separates a formal staff title from the distinct fact of play-calling duty. Head coaches are accepted from game-level nflverse coach fields and compressed into non-overlapping observed-week stints. Checkpoint Eleven-B verifies an explicit offensive-coordinator or primary quarterback-coach listing in an NFL Record & Fact Book as formal-title evidence in a research-only overlay; it does not rewrite the frozen serving assignments or their Checkpoint Six exposure lineage. The evidence row retains `season_designation`, its original source-confidence level, and an open interval review unless dated evidence establishes exact weeks. Dated change reports replace those nominal research rows with non-overlapping `dated_source_weeks` stints. Assistant quarterback coaches are never promoted to the primary role.

Every verified assignment must join to at least one citation. Citations retain the exact HTTPS URL, source title/type, access date, page locator, and a short evidence note. Source confidence is high for nflverse or NFL/club-hosted evidence and medium for an official book available only through a preservation mirror. Play callers require explicit evidence of actual responsibility and a defensible interval; no HC or OC title is used as a proxy. Live-source checks require the all-team caller context plus assignment-specific team/name terms, or direct change language, rather than accepting HTTP availability alone. Houston keeps separate content contracts for Weeks 1–3, shared Week 4, the preseason later designation, and the post-Week-4 boundary. Its Weeks 5–17 allocation remains provisional because the public evidence does not resolve the exact later weekly split.
...
Checkpoint Eleven-B applies that unchanged contract to the strengthened evidence. Its ignored,
content-addressed output adds a 512-cell-per-role coverage matrix, an exhaustive unresolved-caller
artifact, season attribution reconciliation, PAE joinability, and repeatability-readiness counts.
PAE observations retain the complete `(data_version, player_id, team_id, season)` key before a
verified team-season coaching fact is attached. Formal OC/QB-coach titles can support a
team-season PAE linkage even when exact weekly continuity remains under review; PCAE attribution
is stricter and accepts only verified, non-shared, non-`season_designation` caller intervals.
