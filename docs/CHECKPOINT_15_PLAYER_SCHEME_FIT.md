...
- Checkpoint 15 version: `c15-2ac7553234223553`
- Baseline: `505fd184bab5f1e0425cf68fba966d2c8652c1bb`
- Player × Scheme Fit status: **NOT SUPPORTED**
- Standalone fit quantity: **not justified**
- Checkpoint 16 readiness: **READY for M0/M1 research only**
- Checkpoint 16 may consume Player × Scheme interactions: **no**

This is a completed negative research result, not a failed build. The repository cannot establish
...
14 Player State remains team-independent. The target team is attached separately only through:
...
Conflicting teams on the same most-recent snapshot are `AMBIGUOUS_PRESEASON_TEAM`. Undated and
post-cutoff records cannot establish a target team. Target outcomes are joined only after the
entering-season cohort is frozen, on the complete `(player_id, target_team_id, target_season)`
key. Multi-team outcomes therefore cannot cross-attach or retroactively select a team.
...
Primary scheme features are prior-environment shotgun, no-huddle, pass, early-down pass, neutral
pass, short/intermediate/deep target, average air-yards, and scramble rates. Expected pass rate and
PROE are conditional challengers. These are historical Scheme Engine values, not target-season
realized behavior and not causal coach characteristics.
...
The 8,032-row Player State cohort contains 130 evaluation-eligible target-team rows: 77 from
immutable draft-team evidence and 53 from dated 2025 depth charts. Exclusions are 7,767
...
Checkpoint 16 may reuse the leakage-safe cohort contract and benchmark M0 against M1, but it must
not consume M2 coefficients, an interaction contribution, a standalone fit quantity, or a Fit
Score from this checkpoint. A broader timestamped historical preseason team-assignment source is
required before Player × Scheme interactions can be evaluated defensibly.

## Deterministic outputs

Artifacts publish atomically beneath
`data/processed/player_scheme_fit/c15-2ac7553234223553/` and remain ignored by Git. The identity
hashes Checkpoints 13/14, source facts, all dated depth-chart inputs, feature/model/cohort
...
The structured outputs are the fit registry, modeling cohort, preseason team assignments,
...
builds must have the same version and byte-identical deterministic artifacts.
