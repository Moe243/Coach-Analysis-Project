...
FastAPI connects through `DATABASE_URL`, marks every request transaction read-only, and queries only current-publication views/tables. Whitelisted sorting with complete business-key tie-breakers, bound parameters, limit 1-200, nonnegative offsets, typed role/status validation, 404 details, and empty pages form API contract `api-v1.5`. It exposes canonical-position QB facts through `api_qb_statistics`, team-season results/offense through `api_team_season_statistics`, a role-complete coaching audit, and timing-safe inherited environment rows. The audit stores named-person and verified-no-designated-role states separately; the latter carries source and interval evidence without creating an assignment. Relationship Explorer reads that audit in one bounded query for visible team-seasons and renders no-role state outside the node/edge graph. OpenAPI documents the routes. The API remains intentionally unauthenticated, permits CORS only from the exact frontend origin, and exposes no mutation routes.
