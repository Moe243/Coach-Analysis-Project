...
FastAPI connects through `DATABASE_URL`, marks every request transaction read-only, and queries only current-publication views/tables. Whitelisted sorting with complete business-key tie-breakers, bound parameters, limit 1-200, nonnegative offsets, typed role/status validation, 404 details, and empty pages form local Stage 1 API contract `api-v1.4`. It exposes canonical-position QB facts through `api_qb_statistics`, team-season results/offense through `api_team_season_statistics`, a role-complete coaching audit, and timing-safe inherited environment rows. OpenAPI documents the routes. The deployed API remains the approved prior publication until an explicit later release. It remains intentionally unauthenticated, permits CORS only from the exact frontend origin, and exposes no mutation routes.

`GET /relationships/explorer` returns deterministic `Coach -> Team-Season <- QB` facts without adding a duplicate analytical table. Coach nodes retain canonical `coach_id`; QB nodes retain GSIS `player_id`; team-season nodes serialize `(team_id, season)`. Coach relationships remain one sourced `assignment_key` interval, while QB relationships remain one `(player_id, team_id, season)` record and left-join PAE on that complete key. Team-history and team-anchored Full Network scopes come directly from authoritative QB statistics as well as filtered assignments; role, verification, and provisional filters therefore affect coach edges only. Coach, QB, and team-history modes require their corresponding identity. Full Network accepts an optional anchor and can return the complete 2010–2025 scope. Ordinary modes enforce 1,000 canonical nodes and 2,000 factual relationships; Full Network uses measured caps of 2,000 and 4,000. The endpoint never truncates and stamps every relationship with the active publication ID, eliminating frontend N+1 PAE requests.
...
No display substitutes zero for a missing value. Verification, confidence, interval basis, provisional/shared flags, identification, suppression, bootstrap support, and model/data versions remain first-class fields. Coach effects are called exploratory associations and are never presented as definitive rankings. The `/network` route renders Coach Journey, QB Journey, Team History, or Full Network. The first three provide Timeline and deterministic Tree views with season-specific frontend appearance nodes on a fixed vertical chronology; Full Network uses fixed year bands. Underlying API identities and relationship grains remain canonical. Dotted identity-continuity edges are frontend-only navigation semantics, never assignments. Clicking an appearance selects every visible appearance with the same canonical ID and its branch context; selection survives reconstruction when visible and clears when filtered out.
